  ...
  return (
    <div className={`relative rounded-2xl bg-[#2C2C3A] p-6 shadow-lg flex flex-col justify-between hover:scale-[1.01] transition-all w-full duration-300 ease-in-out`}>
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="text-xl font-semibold text-[#F1F1F5] mb-1">{note.title}</h2>
          ...
          {!note.isComplete && (
            <>
              ...
              <div className="relative">
                ...
                {showPriorityMenu && (
                  <div className="absolute top-8 right-0 z-[1000] bg-[#1E1E2F] border border-gray-600 rounded-lg shadow-lg w-36 text-sm">
                    <button onClick={() => handlePriorityChange('high')} className="w-full text-left px-4 py-3 hover:bg-[#FF6B6B]/20 text-[#FF6B6B]">🔥 High</button>
                    <button onClick={() => handlePriorityChange('medium')} className="w-full text-left px-4 py-3 hover:bg-[#FFA94D]/20 text-[#FFA94D]">⚡ Medium</button>
                    <button onClick={() => handlePriorityChange('low')} className="w-full text-left px-4 py-3 hover:bg-[#63E6BE]/20 text-[#63E6BE]">🌿 Low</button>
                  </div>
                )}
              </div>
            </>
